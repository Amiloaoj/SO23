...
int tfs_open(char const *name, tfs_file_mode_t mode) {
    ...
    if (!valid_pathname(name)) {
        printf("tfs_open: invalid path name");
        return -1;
    }
    ...
    if (inum >= 0) {
        ...
        if (inum == -1) {
            printf("tfs_open: no space in inode table");
        ...
        if (add_dir_entry(root_dir_inode, name + 1, inum) == -1) {
            inode_delete(inum);
            printf("tfs_open: no space in directory");
            return -1; // no space in directory
        }

        offset = 0;
    } else {
        printf("tfs_open: file does not exist");
        return -1;
    }
...
int tfs_link(char const *target, char const *source_file) {
    (void)target;
    (void)source_file;
